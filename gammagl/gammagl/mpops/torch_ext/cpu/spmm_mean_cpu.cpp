...
std::tuple<torch::Tensor, torch::Tensor> spmm_mean_cpu_forward(
...
torch::Tensor spmm_mean_cpu_backward(
    torch::Tensor &index, torch::Tensor &weight, torch::Tensor &grad,
    torch::Tensor &messages_count) {
...
#pragma omp parallel for
  ...
  for (auto e = 0; e < E; ++e) {
    ...
    for (auto k = 0; k < K; ++k) {
      auto grad_contribution = grad_data[dst * K + k] /
                               messages_count[dst].item<int64_t>() *
                               weight_data[e];
#ifdef COMPILE_WITH_OMP
#pragma omp atomic
#endif
