...
class WikiCS(InMemoryDataset):
    ...
    def process(self):
        ...
        train_mask = np.ascontiguousarray(np.array(data['train_masks'], dtype=bool).T)
        val_mask = np.ascontiguousarray(np.array(data['val_masks'], dtype=bool).T)
        test_mask = np.ascontiguousarray(np.array(data['test_mask'], dtype=bool).T)
        stopping_mask = np.ascontiguousarray(np.array(data['stopping_masks'], dtype=bool).T)
