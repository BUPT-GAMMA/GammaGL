...
from examples.egt.data.dataset_base import DatasetBase
from examples.egt.data.graph_dataset import GraphDataset
from examples.egt.data.svd_encodings_dataset import SVDEncodingsGraphDataset
from examples.egt.data.structural_dataset import StructuralDataset
...
class PCQM4Mv2(InMemoryDataset):
    def __init__(self, root: str = None,
        ...
        self.dataset_processor = PCQM4Mv2Dataset(
            dataset_path=self.dataset_path, split=split)
    ...
    def record_tokens(self):
        if not hasattr(self, '_dataset'):
            self._dataset = PCQM4Mv2Dataset(
                dataset_path=self.dataset_path, split=self.split)

        tokens = self._dataset.record_tokens
        if not isinstance(tokens, (list, np.ndarray)):
            raise TypeError(
                f"Expected record_tokens to be list/array, got {type(tokens)}")
        return tokens
            ...
            if graph is not None:
                ...
                try:
                    edge_index = tlx.convert_to_tensor(
                        graph['edges'].T, dtype=tlx.int64)
                    x = tlx.convert_to_tensor(
                        graph['node_features'], dtype=tlx.float32)
                    y = tlx.convert_to_tensor(
                        graph['target'], dtype=tlx.float32)
                    edge_attr = tlx.convert_to_tensor(
                        graph['edge_features'], dtype=tlx.float32)

                    data = Graph(edge_index=edge_index, x=x,
                                 y=y, edge_attr=edge_attr)
        ...
        try:
            ...
            with FileLock(lock_path):
                if not hasattr(self, '_dataset'):
                    ...
                    self._dataset = PCQM4Mv2Dataset(
                        root=self.dataset_path, only_smiles=True)
