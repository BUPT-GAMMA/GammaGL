...
class Twitter(InMemoryDataset):
    ...
    url = 'https://www.dropbox.com/scl/fi/uqiglprqpz6rytaoc4k5p/twitter.zip?rlkey=b8f9cltuus36hr0haqt0988o8&st=0cqjc9qx&dl=1'
    ...
    def process(self):
        ...
        node_types = ['twitter', 'topics', 'entity']
        ...
        data['twitter'].y = tlx.convert_to_tensor(y, dtype=tlx.int64)

        split = np.load(osp.join(self.raw_dir, 'train_val_test_idx.npz'))
        for name in ['train', 'val', 'test']:
            idx = split[f'{name}_idx']
            mask = np.zeros(data['twitter'].num_nodes, dtype=np.bool_)
            mask[idx] = True
            data['twitter'][f'{name}_mask'] = tlx.convert_to_tensor(mask, dtype=tlx.bool)


        s = {}
        N_m = data['twitter'].num_nodes
