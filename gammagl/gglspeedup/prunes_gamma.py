...
def _tensor_numel(t):
    return int(np.prod(tlx.get_tensor_shape(t)))
...
class BasePruningMethod(ABC):
    ...
    def apply(cls, module, name, *args, importance_scores=None, **kwargs):
        ...
        if not hasattr(module, name + "_orig"):
            setattr(module, name + "_orig", tlx.convert_to_tensor(orig))
...
class RandomUnstructured(BasePruningMethod):
    ...
    def compute_mask(self, t, default_mask):
        ...
        if nparams_toprune != 0:
            ...
            flat_mask = tlx.scatter_update(flat_mask, topk_indices,
                                           tlx.zeros_like(topk_indices, dtype=flat_mask.dtype))
