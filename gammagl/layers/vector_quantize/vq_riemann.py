...
def _sanitize_inplace(param):
    if isinstance(param, torch.nn.Parameter):
        param.data = _sanitize(param.data)
    elif isinstance(param, torch.Tensor):
        return _sanitize(param)
...
def manifold_norm(manifold, t):
    sign = -1. if isinstance(manifold, Lorentz) else 1.
    norm = sign * manifold.inner(None, t, keepdim=True)
    norm = tlx.sqrt(tlx.maximum(tlx.abs(norm), tlx.convert_to_tensor(1e-8)))
    t = t / (norm)
    return t
...
def ema_inplace(old, new, decay):
    """EMA update that works with both TensorFlow and PyTorch backends."""
    if _has_nan(new):
        return
    backend = getattr(tlx, "BACKEND", None)
    if backend == "tensorflow":
        ...
        # PyTorch: use copy_ for in-place update
...
def uniform_init(manifold, *shape):
    t = manifold.random_normal(*shape, std=0.1)
    return manifold.projx(t)
...
def batched_bincount(x, *, minlength):
    ...
    if isinstance(x, torch.Tensor):
        target = torch.zeros((batch, minlength), dtype=dtype, device=x.device)
        x_long = x.to(dtype=torch.long)
        return target.scatter_add_(1, x_long, torch.ones_like(x, dtype=dtype, device=x.device))
    else:
...
def kmeans(
    ...
    for _ in range(num_iters):
        ...
        if isinstance(samples, torch.Tensor):
            new_means = torch.zeros((num_codebooks, num_clusters, dim), dtype=samples.dtype, device=samples.device).scatter_add_(1, expanded_buckets.long(), samples)
        else:
            new_means_np = tlx.zeros((num_codebooks, num_clusters, dim), dtype=dtype)
            for h in range(num_codebooks):
                for n in range(samples.shape[1]):
                    cluster_idx = int(tlx.convert_to_numpy(buckets[h, n]))
                    new_means_np[h, cluster_idx] = new_means_np[h, cluster_idx] + samples[h, n]
            new_means = new_means_np
...
def orthogonal_loss_fn(manifold, t):
    h, n = t.shape[:2]
    normed_codes = manifold_norm(manifold, t)
...
class EuclideanCodebook(tlx.nn.Module):
    def __init__(
        ...
        embed = uniform_init(None, num_codebooks, codebook_size, dim) if not kmeans_init else tlx.zeros((num_codebooks, codebook_size, dim))
    ...
    def init_embed_(self, data, mask = None):
        ...
        if isinstance(self.embed, torch.nn.Parameter):
            self.embed = torch.nn.Parameter(embed)
        else:
            self.embed = embed
    ...
    def replace(self, batch_samples, batch_mask):
        for ind, (samples, mask) in enumerate(zip(batch_samples.unbind(dim = 0), batch_mask.unbind(dim = 0))):
            ...
            if self.learnable_codebook:
                self.embed = tlx.tensor_scatter_nd_update(self.embed, [(ind, mask)], [sampled])
            else:
                self.embed = tlx.where(
                    rearrange(mask, '... -> ... 1'),
                    tlx.broadcast_to(sampled, self.embed.shape),
                    self.embed
                )
            self.embed_avg = tlx.where(
                rearrange(mask, '... -> ... 1'),
                sampled * self.reset_cluster_size,
                self.embed_avg
            )
            self.cluster_size = tlx.where(
                mask,
                tlx.ones_like(self.cluster_size) * self.reset_cluster_size,
                self.cluster_size
            )
    ...
    def forward(
        ...
        if not self.learnable_codebook:
            ...
            if isinstance(self.embed, torch.Tensor):
                is_nan = bool(torch.isnan(self.embed).any())
            else:
                is_nan = bool(np.isnan(np.asarray(embed_arr)).any())
            if is_nan:
                print("[VQ-FIX] Riemann EuclideanCodebook: NaN detected in codebook embed, resetting...")
                self.initted = tlx.convert_to_tensor([False])
                self.cluster_size = tlx.zeros_like(self.cluster_size)
                init_embed = uniform_init(None, self.num_codebooks, self.codebook_size, self.embed.shape[-1])
                self.embed_avg = init_embed
                self.embed = tlx.convert_to_tensor(init_embed)
                self.init_embed_(flatten, mask = mask)

        embed = self.embed if self.learnable_codebook else stop_gradient(self.embed)

        dist = -manifold.dist(flatten, embed)
...
class CosineSimCodebook(tlx.nn.Module):
    def __init__(
        ...
        super().__init__()
        self.transform_input = manifold_norm
        self.manifold = manifold
        self.ema_update = ema_update
        self.decay = decay

        if not kmeans_init:
            embed = manifold_norm(self.manifold, uniform_init(self.manifold, num_codebooks, codebook_size, dim))
        else:
            embed = tlx.zeros((num_codebooks, codebook_size, dim))
        ...
        self.initted = tlx.convert_to_tensor([not kmeans_init])
        self.cluster_size = tlx.zeros((num_codebooks, codebook_size, dim))
        self.embed_avg = embed
    ...
    def replace(self, batch_samples, batch_mask):
        batch_samples = manifold_norm(self.manifold, batch_samples)
    ...
    def forward(
        ...
        self.init_embed_(flatten, mask = mask)
        if _has_nan(self.embed):
            _sanitize_inplace(self.embed)
        if self.learnable_codebook and isinstance(self.embed, torch.Tensor):
            _check("rcsc_embed_before_use", self.embed)
        embed = self.embed if self.learnable_codebook else stop_gradient(self.embed)

        def manifold_pairwise_dist(manifold, x, y):
            x = manifold.projx(x)
            y = manifold.projx(y)
            x = tlx.expand_dims(x, 2)
            y = tlx.expand_dims(y, 1)
        ...
        _check("rcsc_dist", dist)
        # squeeze 掉多余的维度 [h, n, 1, d] -> [h, n, d]
        if len(dist.shape) == 4 and dist.shape[2] == 1:
            dist = tlx.squeeze(dist, axis=2)

        embed_ind, embed_onehot = self.gumbel_sample(dist, dim = -1, temperature = sample_codebook_temp, training = self.is_train)
        embed_ind = unpack_one(embed_ind, ps, 'h *')
        _check("rcsc_embed_onehot", embed_onehot)

        if self.is_train:
            unpacked_onehot = unpack_one(embed_onehot, ps, 'h * c')
            quantize = einsum(unpacked_onehot, embed, 'h b n c, h c d -> h b n d')
            _check("rcsc_quantize", quantize)
        else:
            quantize = batched_embedding(embed_ind, embed)
        ...
        if self.is_train and self.ema_update and not freeze_codebook:
            ...
            if hasattr(embed_sum, 'contiguous'):
                embed_sum = embed_sum.contiguous()
            embed_sum = _sanitize(embed_sum)
            self.all_reduce_fn(embed_sum)

            ema_inplace(self.embed_avg, embed_sum, self.decay)

            cluster_size = laplace_smoothing(self.cluster_size, self.codebook_size, self.eps) * tlx.reduce_sum(self.cluster_size, axis = -1, keepdims=True)
            cluster_size = tlx.maximum(cluster_size, tlx.convert_to_tensor(1e-8, dtype=cluster_size.dtype))

            embed_normalized = self.embed_avg / rearrange(cluster_size, '... -> ... 1')
            embed_normalized = _sanitize(embed_normalized)
            embed_normalized = manifold_norm(self.manifold, embed_normalized)
...
class VectorQuantize_R(tlx.nn.Module):
    def __init__(
        self,
        manifold,
        dim,
        ...
        self.project_in = ConstCurveLinear(manifold, dim, codebook_input_dim) if requires_projection else None
        self.project_out = ConstCurveLinear(manifold, codebook_input_dim, dim) if requires_projection else None
        ...
        self._codebook = codebook_class(manifold, **codebook_kwargs)
    ...
    def forward(
        ...
        if self._codebook.transform_input is not identity:
            x = self._codebook.transform_input(self.manifold, x)
        ...
        quantize, embed_ind, distances = self._codebook(self.manifold, x, **codebook_forward_kwargs)

        if self.is_train and not self.learnable_codebook:
            if isinstance(quantize, torch.Tensor) and torch.isnan(quantize).any():
                print(f"[VQ-FIX] VectorQuantize_R: NaN in codebook output, falling back to input. shape={quantize.shape}")
                quantize = x
        ...
        if self.is_train:
            ...
            # Flatten before Lorentz logmap to avoid high-rank broadcasting blow-up (OOM),
            # then restore the original shape.
            x_shape = x.shape
            x_flat = tlx.reshape(x, (-1, x_shape[-1]))
            quantize_flat = tlx.reshape(quantize, (-1, x_shape[-1]))
            quantize_flat = self.manifold.expmap(
                x_flat,
                self.manifold.proju(x_flat, stop_gradient(self.manifold.logmap(x_flat, quantize_flat)))
            )
            quantize = tlx.reshape(quantize_flat, x_shape)
        ...
        if self.is_train:
            if self.commitment_weight > 0:
                if self.commitment_use_cross_entropy_loss:
                    ...
                    if exists(mask):
                        ...
                        commit_loss = tlx.reduce_mean(tlx.pow(self.manifold.dist(x, commit_quantize), 2))
            ...
            if self.has_codebook_orthogonal_loss:
                ...
                orthogonal_reg_loss = orthogonal_loss_fn(self.manifold, codebook)
        ...
        quantize = manifold_norm(self.manifold, quantize)
