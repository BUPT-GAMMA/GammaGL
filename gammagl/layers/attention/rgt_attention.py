...
def segment_softmax_fast(data, segment_ids, num_segments=None, eps=1e-12):
    """Torch-fast segment softmax without GammaGL C++ extension dependency."""
    if not (isinstance(data, torch.Tensor) and isinstance(segment_ids, torch.Tensor)):
    ...
    segment_ids = segment_ids.to(dtype=torch.long, device=data.device)
    if num_segments is None:
        num_segments = int(torch.max(segment_ids).item()) + 1

    max_values = torch.full((num_segments,), -float("inf"), dtype=data.dtype, device=data.device)
    if hasattr(max_values, "scatter_reduce_"):
        max_values.scatter_reduce_(0, segment_ids, data, reduce="amax", include_self=True)
    else:
        # Fallback for very old torch.
        for i in range(num_segments):
            mask = segment_ids == i
            if torch.any(mask):
                max_values[i] = torch.max(data[mask])

    shifted = data - max_values[segment_ids]
    exp_values = torch.exp(shifted)
    denom = torch.zeros((num_segments,), dtype=data.dtype, device=data.device)
    denom.scatter_add_(0, segment_ids, exp_values)
    return exp_values / (denom[segment_ids] + eps)
    ...
    def forward(self, x_q, x_k, x_v, edge_index, agg_index=None):
        ...
        if isinstance(src, torch.Tensor):
            ...
            if hasattr(src, "detach"):
                ...
                src_np = np.array(src)
            if hasattr(dst, "detach"):
                dst_np = dst.detach().cpu().numpy()
            elif hasattr(dst, "numpy"):
                dst_np = dst.numpy()
            else:
                dst_np = np.array(dst)
            src = tlx.convert_to_tensor(src_np, dtype=tlx.int64)
            dst = tlx.convert_to_tensor(dst_np, dtype=tlx.int64)
        agg_index = agg_index if agg_index is not None else src

        # GammaGL fallback segment_softmax may fail when segment ids have empty gaps.
        # Compact segment ids to contiguous [0, num_unique) for robust softmax.
        ...
        if tlx.BACKEND == 'torch' and isinstance(src, torch.Tensor):
            # Keep segment ids compact to avoid empty segments in fallback mpops path.
            src_compact = torch.unique(src, sorted=True, return_inverse=True)[1]
            score = segment_softmax_fast(score, src_compact, num_segments=int(torch.max(src_compact).item()) + 1)
        else:
            src_np = tlx.convert_to_numpy(src)
            src_compact = tlx.convert_to_tensor(np.unique(src_np, return_inverse=True)[1], dtype=tlx.int64)
            score = segment_softmax(score, src_compact, num_segments=int(tlx.convert_to_numpy(tlx.reduce_max(src_compact))) + 1)
