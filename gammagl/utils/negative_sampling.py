...
def sample(population, k, device):
    if population <= k:
        return tlx.convert_to_tensor(np.arange(population), device=device)
    else:
        return tlx.convert_to_tensor(random.sample(range(population), k), device=device)
