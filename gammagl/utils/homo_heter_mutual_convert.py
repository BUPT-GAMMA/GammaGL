...
def to_homograph(x_dict ,edge_index_dict, num_nodes_dict, edge_value_dict):
    ...
    for i in node_type_list[1:]:
        if(tlx.backend=="tensorflow"):
            ...
            x_feature = tlx.concat((x_feature,x_dict[i]), axis=0)
    ...
    if(tlx.backend=="tensorflow"):
        node_src = tlx.stack(node_src)
        node_dst = tlx.stack(node_dst)
    edge_index = tlx.concat((node_src,node_dst),axis=0)
    if(edge_value_dict!=None):
        edge_value = edge_value_dict[edge_type_list[0]]
        if(tlx.backend=="tensorflow"):
            edge_value[0]=tlx.stack(edge_value[0])
    ...
    for i in edge_type_list[1:]:
        ...
        if(tlx.backend=="tensorflow"):
            node_src = tlx.stack(node_src)
            node_dst = tlx.stack(node_dst)
        edge_index_tem = tlx.concat((node_src,node_dst),axis=0)
        if(tlx.backend=="tensorflow"):
            edge_index_tem = tlx.stack(edge_index_tem)
        edge_index = tlx.concat((edge_index,edge_index_tem),axis=1)
        if(edge_value_dict!= None):
            if(tlx.backend=="tensorflow"):
                edge_value = tlx.concat((edge_value,tlx.stack(edge_value_dict[i][0])),axis=0)
            else:
                edge_value = tlx.concat((edge_value,edge_value_dict[i][0]),axis=0)
        else:
            edge_value = None
