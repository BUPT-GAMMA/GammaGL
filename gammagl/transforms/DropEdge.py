import numpy
from gammagl.transforms import BaseTransform
from scipy.stats import bernoulli
from gammagl.data import graph
import tensorlayerx as tlx
class DropEdge(BaseTransform):
    r"""Randomly drop edges, as described in
        `DropEdge: Towards Deep Graph Convolutional Networks on Node Classification
        <https://arxiv.org/abs/1907.10903>`__.

        Parameters
        ----------
        p : float, optional
            Probability of an edge to be dropped.
        Example
        -------

        >>> from gammagl.data import graph
        >>> from gammagl.transforms import DropEdge
        >>> import tensorlayerx as tlx
        >>> transform = DropEdge()
        >>> g = graph.Graph(x=numpy.random.randn(5, 16), edge_index=tlx.constant([[0, 0, 0], [1, 2, 3]]), edge_attr=tlx.constant([[0,0,0],[1,1,1],[2,2,2]]),num_nodes=5,)
        >>> new_g = transform(g)
        >>> print(g)
        Graph(edge_index=[2, 3], edge_attr=[3, 3], x=[5, 16], num_nodes=5)
        >>> print(new_g)
        Graph(edge_index=[2, 1], edge_attr=[1, 3], x=[5, 16], num_nodes=5)
        >>> from gammagl.data import HeteroGraph
        >>> data = HeteroGraph()
        >>> num_papers=5
        >>> num_paper_features=6
        >>> num_authors=7
        >>> num_authors_features=8
        >>> data['paper'].x = tlx.random_uniform((num_papers, num_paper_features))
        >>> data['author'].x =  tlx.random_uniform((num_authors, num_authors_features))
        >>> data['author', 'writes', 'paper'].edge_index = tlx.constant([[0, 0, 0], [1, 2, 3]])  # [2, num_edges]
        >>> data['author', 'writes', 'paper'].edge_attr = tlx.constant([[0, 0, 0], [1, 1, 1],[2, 2, 2]])
        >>> data['author', 'writes', 'author'].edge_index = tlx.constant([[1, 2, 3], [2, 3, 1]])
        >>> print(data)
        HeteroGraph(
          paper={ x=[5, 6] },
          author={ x=[7, 8] },
          (author, writes, paper)={
            edge_index=[2, 3],
            edge_attr=[3, 3]
          },
          (author, writes, author)={ edge_index=[2, 3] }
        )
        >>> transform = DropEdge()
        >>> new_data = transform(data)
        >>> print(new_data)
        HeteroGraph(
          paper={ x=[5, 6] },
          author={ x=[7, 8] },
          (author, writes, paper)={
            edge_index=[2, 1],
            edge_attr=[1, 3]
          },
          (author, writes, author)={ edge_index=[2, 2] }
        )

        """
    def __init__(self, p=0.5):
        self.p = p

    def __call__(self, g):
        if self.p == 0:
            return g
        if (type(g)==graph.Graph):
            samples = tlx.ops.constant(bernoulli.rvs(self.p, size=g.num_edges),dtype=bool)
            if g.edge_index==None:
                return g
            elif g.edge_attr==None:
                g.edge_index = tlx.ops.transpose(tlx.ops.transpose(g.edge_index)[samples])

                return g
            else:
                g.edge_index = tlx.ops.transpose(tlx.ops.transpose(g.edge_index)[samples])

                g.edge_attr = g.edge_attr[samples]
                return g
        else:
            for e_type in g.metadata()[-1]:
                samples = tlx.ops.constant(bernoulli.rvs(self.p, size=tlx.ops.get_tensor_shape(g[e_type].edge_index)[-1]), dtype=bool)
                if hasattr(g[e_type],'edge_index'):
                    g[e_type].edge_index = tlx.ops.transpose(tlx.ops.transpose(g[e_type].edge_index)[samples])
                    if hasattr(g[e_type],'edge_attr'):
                        g[e_type].edge_attr = g[e_type].edge_attr[samples]
                    else:
                        pass
                else:
                    pass
            return g









