...
//    assoc = py::cast<Tensor>(put(assoc, idx, arange_arr));
    ...
    // assoc = python_for_c<double_t>("put", assoc, idx, np_func<int64_t>("arange", 10));
    auto assoc = py_helper<int64_t>("new", rowptr.shape(0) - 1, -1);
    assoc = py_helper<int64_t>("put", assoc, idx, np_func<int64_t>("arange", idx.shape(0)));

//    py::gil_scoped_release release{};

    auto idx_data = idx.mutable_unchecked();
    auto rowptr_data = rowptr.mutable_unchecked();
    auto col_data = col.mutable_unchecked();
    auto assoc_data = assoc.mutable_unchecked();
