...
class GEstimationN_tlx():
	...
	def __init__(self, data):
		...
		self.idx_train = mask_to_index(graph.train_mask)
		self.label = graph.y
	...
	def reset_obs(self):
		self.N = 0
		self.E = tlx.zeros((self.num_node, self.num_node), dtype=tlx.int64)
	...
	def E_step(self, Q):
		...
		n = []
		counter = Counter(tlx.convert_to_numpy(self.output))
	...
	def M_step_tl(self, alpha, beta, O):
	...
	def M_step(self, alpha, beta, O):
		...
		row = tlx.convert_to_numpy(self.output).repeat(self.num_node)
		col = tlx.tile(self.output, [self.num_node])
		tmp = tlx.convert_to_tensor(O[row, col].reshape(self.num_node, -1))

		p1 = tmp * tlx.pow(alpha, self.E) * tlx.pow(1 - alpha, self.N - self.E)
		p2 = (1 - tmp) * tlx.pow(beta, self.E) * tlx.pow(1 - beta, self.N - self.E)
		Q = p1 * 1. / (p1 + p2 * 1.)
		return Q
