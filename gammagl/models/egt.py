...
class EGT(EGTModel):
    def __init__(self, **kwargs):
        ...
        if (not self.node_ended) and (not self.edge_ended):
            ...
            self.EGT_layers.append(EGTConv(**self.layer_common_kwargs, node_update = False))
        elif not self.edge_ended:
            self.EGT_layers.append(EGTConv(**self.layer_common_kwargs, edge_update = False))
        else:
...
class EGT_MOL(EGT):
    def __init__(self,
        ...
        if self.svd_encodings:
            self.svd_embed = Linear(out_features=self.node_width, in_features=self.svd_encodings * 2)

        self.dist_embed = Embedding(num_embeddings=self.upto_hop + 2, embedding_dim=self.edge_width)
    ...
    def input_block(self, inputs):
        ...
        nodef = tlx.ops.cast(tlx.convert_to_tensor(g.node_features), dtype=tlx.int64)
        
        nodem = tlx.ops.cast(tlx.convert_to_tensor(g.node_mask), dtype=tlx.float32)
        ...
        featm = tlx.ops.cast(tlx.convert_to_tensor(g.feature_matrix), dtype=tlx.int64)
        
        h = tlx.ops.reduce_sum(self.nodef_embed(nodef), axis=2)    # (b,i,w,h) -> (b,i,h)
        
        if self.svd_encodings:
            h = h + self.svd_embed(g.svd_encodings)
        
        e = self.dist_embed(dm) \
            + tlx.reduce_sum(self.featm_embed(featm), axis=3)  # (b,i,j,f,e) -> (b,i,j,e)
    ...
    def final_embedding(self, g):
        ...
        if self.num_virtual_nodes > 0:
            ...
            nodem = tlx.expand_dims(tlx.cast(g.node_mask, dtype=tlx.float32), axis=-1)
            h = tlx.reduce_sum(h * nodem, axis=1) / (tlx.reduce_sum(nodem, axis=1) + 1e-9)
        g.h = h
        return g
