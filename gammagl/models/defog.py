...
class DeFoGModel(tlx.nn.Module):
    ...
    def __init__(self, n_layers, input_dims, hidden_mlp_dims, hidden_dims,
        ...
        print("[debug:model] Enter DeFoGModel.__init__", flush=True)
        super().__init__(name=name)
        print("[debug:model] After super().__init__", flush=True)
        ...
        # Input MLPs
        print("[debug:model] Building mlp_in_X...", flush=True)
        ...
        print("[debug:model] Built mlp_in_X", flush=True)
        print("[debug:model] Building mlp_in_E...", flush=True)
        ...
        print("[debug:model] Built mlp_in_E", flush=True)
        # +64 for timestep embedding dimension
        print("[debug:model] Building mlp_in_y...", flush=True)
        ...
        print("[debug:model] Built mlp_in_y", flush=True)

        # Transformer layers
        print("[debug:model] Building tf_layers...", flush=True)
        self.tf_layers = tlx.nn.ModuleList()
        for i in range(n_layers):
            print(f"[debug:model] Building transformer layer {i + 1}/{n_layers}...", flush=True)
        ...
        print("[debug:model] Built tf_layers", flush=True)

        # Output MLPs
        print("[debug:model] Building mlp_out_X...", flush=True)
        ...
        print("[debug:model] Built mlp_out_X", flush=True)
        print("[debug:model] Building mlp_out_E...", flush=True)
        ...
        print("[debug:model] Built mlp_out_E", flush=True)
        print("[debug:model] Building mlp_out_y...", flush=True)
        ...
        print("[debug:model] Built mlp_out_y", flush=True)
        print("[debug:model] DeFoGModel.__init__ complete", flush=True)
