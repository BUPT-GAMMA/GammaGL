...
class DatasetBase:
    ...
    def cache_load_and_save(self, base_path, op, verbose):
        ...
        if op == 'load':
            ...
            self._record_tokens = tlx.files.load_npy_to_any(name=str(tokens_path))
            ...
            tlx.files.save_any_to_npy(save_dict=self.record_tokens, name=str(tokens_path))
            tlx.files.save_any_to_npy(save_dict=self._records, name=str(records_path))
        else:
