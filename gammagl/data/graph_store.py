r"""This class defines the abstraction for a backend-agnostic graph store.
Thr goal of the graph store is to abstract away alla graph edge index memory
management so that vbarying implementations can allow for independent scale-out.

This particular graph store abstraction makes a few key assumptions:
* The edge indices we care about storing are represented either in COO, CSC, or
  CSR format. They can be uniquely identified by an edge type (in GammaGL,
  this is a tuple of the form (source node type, edge type, destination node)).
* The edge indices are static once they are stored in the graph. That is, we do not 
...
ConversionOutputType = Tuple[Dict[EdgeType, Tensor], Dict[EdgeType, Tensor], Dict[EdgeType, Tensor]]
...
class GraphStore(ABC):
    ...
    def put_edge_index(self, edge_index: EdgeTensorType, *args,
        ...
        Args:
            edge_index (Tuple[Tensor, Tensor]): The:obj:
                `edge_index` tuple in a format specified in
                :class:`EdgeAttr`.
