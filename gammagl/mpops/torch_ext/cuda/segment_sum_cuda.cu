...
__global__ void segment_sum_cuda_forward_kernel(const scalar_t *x_data, const int64_t *index_data,
...
template <typename scalar_t>
__global__ void
arg_segment_sum_cuda_forward_kernel(const scalar_t *x_data, const int64_t *index_data,
...
segment_sum_cuda_forward(torch::Tensor x, torch::Tensor index, int64_t N) {
  // check inputs
  ...
  arg_segment_sum_cuda_forward_kernel<scalar_t>
      <<<BLOCKS(x.numel()), THREADS, 0, stream>>>(
