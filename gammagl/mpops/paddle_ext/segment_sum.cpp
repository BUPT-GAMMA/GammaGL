...
std::vector<paddle::Tensor> SegmentSumBackward(
    const paddle::Tensor& index, const paddle::Tensor& grad_out) {
...
std::vector<std::vector<int64_t>> SegmentForwardShape(
    ...
    const std::vector<int64_t>& index_shape, const int64_t& n) {
  return {{n, x_shape[1]}};
}
std::vector<std::vector<int64_t>> SegmentBackwardShape(
    const std::vector<int64_t> x_shape, const std::vector<int64_t>& index_shape,
    const std::vector<int64_t>& grad_out_shape) {
  return {x_shape};
}
std::vector<paddle::DataType> SegmentForwardDtype(
    paddle::DataType x_dtype, paddle::DataType index_dtype) {
  return {x_dtype};
}
