...
__global__ void segment_sum_cuda_forward_kernel(
    const data_t *x_data, const int64_t *index_data, data_t *out_data,
...
std::vector<paddle::Tensor> segment_sum_cuda_forward(
  ...
  segment_sum_cuda_forward_kernel<data_t>
      <<<BLOCKS(x.numel()), THREADS, 0, x.stream()>>>(
          x.data<data_t>(), index.data<int64_t>(), out.data<data_t>(), E, K, n,
          x.numel());
  // }));
