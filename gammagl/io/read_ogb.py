...
def read_graph(raw_dir, add_inverse_edge=False, additional_node_files=[], additional_edge_files=[], binary=False):
    ...
    for graph in tqdm(graph_list):
        ...
        g.edge_index = graph['edge_index']

        del graph['num_nodes']
        del graph['edge_index']

        if graph['edge_feat'] is not None:
            g.edge_attr = graph['edge_feat']
            del graph['edge_feat']
...
def read_heterograph(raw_dir, add_inverse_edge=False, additional_node_files=[], additional_edge_files=[],
    ...
    for graph in tqdm(graph_list):
        ...
        for triplet, edge_index in graph['edge_index_dict'].items():
            g.edge_index_dict[triplet] = edge_index
        ...
        if graph['edge_feat_dict'] is not None:
            ...
            for triplet in graph['edge_feat_dict'].keys():
                g.edge_attr_dict[triplet] = graph['edge_feat_dict'][triplet]
        ...
        if graph['node_feat_dict'] is not None:
            ...
            for nodetype in graph['node_feat_dict'].keys():
                g.x_dict[nodetype] = graph['node_feat_dict'][nodetype]
        ...
        for key in additional_node_files:
            ...
            for nodetype in graph[key].keys():
                g[key][nodetype] = graph[key][nodetype]
        ...
        for key in additional_edge_files:
            ...
            for triplet in graph[key].keys():
                g[key][triplet] = graph[key][triplet]
...
def read_node_label_hetero(raw_dir):
    ...
    for nodetype in df.keys():
        ...
        if has_label:
            label_dict[nodetype] = pd.read_csv(osp.join(raw_dir, 'node-label', nodetype, 'node-label.csv.gz'),
...
def read_nodesplitidx_split_hetero(split_dir):
    ...
    for nodetype in df.keys():
        ...
        if has_label:
            train_dict[nodetype] = \
            pd.read_csv(osp.join(split_dir, nodetype, 'train.csv.gz'), compression='gzip', header=None).values.T[0]
            valid_dict[nodetype] = \
            pd.read_csv(osp.join(split_dir, nodetype, 'valid.csv.gz'), compression='gzip', header=None).values.T[0]
            test_dict[nodetype] = \
            pd.read_csv(osp.join(split_dir, nodetype, 'test.csv.gz'), compression='gzip', header=None).values.T[0]
