...
def test_dfde_norm_g_all_edges_dropped():
    x = tlx.convert_to_tensor(np.array([[1, 0], [0, 1], [1, 1], [0, 0]], dtype=np.float32))
    edge_index = tlx.convert_to_tensor(np.array([[0, 1, 2, 3], [1, 2, 3, 0]], dtype=np.int64))
    new_g = dfde_norm_g(edge_index, x, feat_drop_rate=0.0, drop_edge_rate=1.0)

    assert new_g.edge_index.shape == (2, x.shape[0])
    assert new_g.edge_weight.shape[0] == new_g.edge_index.shape[1]
