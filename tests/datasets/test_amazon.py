...
def test_amazon_dataset():
    ...
    edge_num=dataset_computers.data["edge_index"].shape[1]
    assert edge_num==491722
    node_num=dataset_computers.data["x"].shape[0]
    ...
    edge_num=dataset_photo.data["edge_index"].shape[1]
    assert edge_num==238162
    node_num=dataset_photo.data["x"].shape[0]
