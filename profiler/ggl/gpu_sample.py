...
def main(args):
    ...
    train_loader = NeighborSampler(edge_index=graph.edge_index,
                                   node_idx=train_idx,
                                   sample_lists=[8, 8, 4], batch_size=4096, shuffle=True, num_workers=0)
    ...
    for epoch in range(args.n_epoch):
        ...
        up_epoch_time = time.time()
        for dst_node, n_id, adjs in train_loader:
            sample_time = sample_time + time.time() - up_epoch_time
            net.set_train()
