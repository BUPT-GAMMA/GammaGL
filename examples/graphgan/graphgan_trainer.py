...
from gammagl.utils import write_embeddings_to_file, sample, read_embeddings
...
def main(args):
    data = CA_GrQc(args.dataset_path, args.n_emb)

    GANModel = GraphGAN(data.n_node, data.graph, data.node_embed_init_d, data.node_embed_init_g, args.cache_folder, args.multi_processing)
    ...
    calculate_acc(GANModel.n_node, data.test_edges, data.test_edges_neg, args)
    ...
    for epoch in range(args.n_epochs):
        ...
        for d_epoch in range(args.n_epochs_dis):
            ...
            for start in start_list:
                ...
                train_one_step_d(input, labels_sets)
        ...
        for g_epoch in range(args.n_epochs_gen):
            ...
            for start in start_list:
                ...
                train_one_step_g(input, reward_sets)

        write_embeddings_to_file(GANModel, args, 1)
        g_val_acc, d_val_acc = calculate_acc(GANModel.n_node, data.test_edges, data.test_edges_neg, args)
...
if __name__ == '__main__':
    ...
    # Path settings
    parser.add_argument("--dataset_path", type=str, default='gan_dataset', help="path to save dataset")
