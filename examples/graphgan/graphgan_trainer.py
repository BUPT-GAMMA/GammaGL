...
from gammagl.utils import read_embeddings, read_edges_from_file, write_embeddings_to_file, sample
...
def calculate_acc(n_node, args):
   ...
   Returns:
       g_val_acc: accuracy of generator
       d_val_acc: accuracy of discriminator
...
def prepare_data_for_d(GANModel, args):
    ...
    for i in GANModel.root_nodes:
        if np.random.rand() < args.update_ratio:
            ...
            neg, _ = sample(all_score_ndarray, i, GANModel.trees[i], len(pos), for_d=True)
...
def prepare_data_for_g(GANModel, args):
    ...
    for i in GANModel.root_nodes:
        if np.random.rand() < args.update_ratio:
            sample_nodes, paths_from_i = sample(all_score_ndarray, i, GANModel.trees[i], args.n_sample_gen, for_d=False)
...
def main(args):
    ...
    train_one_step_d = tlx.model.TrainOneStep(net_with_loss_D, optimizer_d, d_weights)
    train_one_step_g = tlx.model.TrainOneStep(net_with_loss_G, optimizer_g, g_weights)
    ...
    if args.load_model:
        ...
        if d_num and g_num:
            ...
            GANModel.discriminator.load_weights(file_path="checkpoint/model_d" + str_d + ".npz", format='npz_dict')
            GANModel.generator.load_weights(file_path="checkpoint/model_g" + str_g + ".npz", format='npz_dict')

    write_embeddings_to_file(GANModel, args, 1)
    calculate_acc(GANModel.n_node, args)
    ...
    for epoch in range(args.n_epochs):
        ...
        if epoch > 0 and epoch % args.save_steps == 0:
            ...
            store_path_d = 'checkpoint/model_d-0000' + str(store_num_d) + '.npz'
            store_path_g = 'checkpoint/model_g-0000' + str(store_num_g) + '.npz'
            GANModel.discriminator.save_weights(file_path=store_path_d, format='npz_dict')
            GANModel.generator.save_weights(file_path=store_path_g, format='npz_dict')
        ...
        for d_epoch in range(args.n_epochs_dis):
            print("d_epoch %d" % (d_epoch + 1))
            # Generate new nodes for the D for every dis_interval iterations
            if d_epoch % args.dis_interval == 0:
                center_nodes, neighbor_nodes, labels = prepare_data_for_d(GANModel, args)
        ...
        write_embeddings_to_file(GANModel, args, 1)
        g_val_acc, d_val_acc = calculate_acc(GANModel.n_node, args)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--n_epochs", type=int, default=30, help="number of outer loops")
    ...
    parser.add_argument("--lr_gen", type=float, default=1e-5, help="learning rate for the generator")
    parser.add_argument("--lr_dis", type=float, default=1e-5, help="learning rate for the discriminator")
    ...
    parser.add_argument("--n_emb", type=int, default=50, help="the dimension of node embeddings")
    ...
    # Path settings
    parser.add_argument("--train_filename", type=str, default='gan_dataset/' + dataset + '_train.txt',
                        help="training dataset filename")
    parser.add_argument("--pre_train_emb_filename_d", type=str,
                        default='gan_dataset/' + dataset + '_pre_train.emb',
                        help="pre_trained embeddings of discriminator filename")
    parser.add_argument("--pre_train_emb_filename_g", type=str,
                        default='gan_dataset/' + dataset + '_pre_train.emb',
                        help="pre_trained embeddings of generator filename")
    parser.add_argument("--test_filename", type=str, default='gan_dataset/' + dataset + '_test.txt',
                        help="test dataset filename")
    parser.add_argument("--test_neg_filename", type=str,
                        default='gan_dataset/' + dataset + '_test_neg.txt',
                        help="negative samples for test filename")
    parser.add_argument("--emb_filenames", type=list,
                        default=['gan_results/' + dataset + '_gen_.emb',
                                 'gan_results/' + dataset + '_dis_.emb'],
                        help="embeddings during training filenames")
    parser.add_argument("--best_acc_emb_filenames", type=list,
                        default=['gan_results/' + dataset + '_best_acc_gen_.emb',
                                 'gan_results/' + dataset + '_best_acc_dis_.emb'],
                        help="embeddings with the best accuracy filenames")
    parser.add_argument("--result_filename", type=str,
                        default='gan_results/' + dataset + '.txt',
