...
def random_aug(graph, attr, diag_attr, x, feat_drop_rate, edge_mask_rate):
    edge_mask = mask_edge(graph, edge_mask_rate)  # randomly drop edegs
    ...
    attr = tlx.concat([attr[edge_mask], diag_attr], axis=-1)
...
def mask_edge(graph, mask_prob):
    ...
    masks_idx= list(np.nonzero(masks)[0])
    return masks_idx
