...
def distill_train(dur, epoch, model, configs, graph, dataset, temp, nei_entropy, t_model, teacher_logits, train_mask,
    ...
    optimizer = tlx.optimizers.Adam(lr=configs['my_lr'])
    train_weights = model.trainable_weights

    loss_func = SemiSpvzLoss(model, distill_loss, graph, configs, nei_entropy,
                             t_model, teacher_logits, dataset.num_classes, train_mask)
    train_one_step = TrainOneStep(loss_func, optimizer, train_weights)
    model.set_train()
    train_loss = train_one_step(graph.x, graph.y)

    t_optimizer = tlx.optimizers.Adam(lr=configs['my_t_lr'])
    t_train_weights = t_model.trainable_weights

    t_loss_func = t_SemiSpvzLoss(t_model, evaluate_loss, graph, configs, dataset.num_classes, model, val_mask)
    t_train_one_step = TrainOneStep(t_loss_func, t_optimizer, t_train_weights)
    t_train_loss = torch.tensor(0)
    if epoch > 20:
        t_model.set_train()
        t_train_loss = t_train_one_step(graph.x, graph.y)
    ...
    loss_val = 0
    print('Epoch %d | acc_val: %.4f | distill_loss: %.4f | evaluate_loss: %.4f' % (epoch, acc_val.item(), train_loss.item(), t_train_loss.item()))
...
def model_train(configs, model, graph, teacher_logits, dataset):
    ...
    print("train_val_my %d %d %d" % (torch.sum(train_mask).item(), torch.sum(val_mask).item(), torch.sum(my_val_mask).item()))
