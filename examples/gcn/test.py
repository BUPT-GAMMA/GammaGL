...
log = 'log_pubmed_torch.txt'
result = 'result_pubmed_torch.csv'
    ...
    for lc in l2_coefs:
        for dr in drop_rate:
            ...
            for ite in iterations:
                ...
                sbp.run('python gcn_trainer.py --dataset pubmed --lr {} --l2_coef {} --drop_rate {} >> {}'.format( lr, lc, dr, log), shell=True)
...
path = 'log_pubmed_torch.txt'
train_num = 5
cora = pd.read_csv(path, chunksize = train_num) #, chunksize = train_num
auc_list = np.array([])
...
result.to_csv('result_pubmed_torch.csv')