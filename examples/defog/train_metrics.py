...
class KLDMetric:
    ...
    def _compute_loss(self, preds, targets):
        ...
        # Sum over feature dimension, mean over batch
        loss = per_sample.sum(dim=-1).mean()
        return float(loss.item())
...
class TrainLossDiscrete(tlx.nn.Module):
    ...
    def _masked_loss(self, metric, preds, targets, mask):
        ...
        preds_m = preds[mask_bool]
        targets_m = targets[mask_bool]

        if isinstance(metric, CrossEntropyMetric):
            if len(targets_m.shape) > 1 and targets_m.shape[-1] > 1:
                targets_m = tlx.argmax(targets_m, axis=-1)
            targets_m = tlx.cast(targets_m, tlx.int64)
            import torch.nn.functional as F
            loss = F.cross_entropy(preds_m, targets_m)
            metric.update_precomputed(float(loss.detach().item()), int(mask_bool.sum().item()))
            return loss
            
        elif isinstance(metric, KLDMetric):
            import torch.nn.functional as F
            log_preds = F.log_softmax(preds_m, dim=-1)
            targets_f = targets_m.float()
            per_sample = F.kl_div(log_preds, targets_f, reduction='none')
            loss = per_sample.sum(dim=-1).mean()
            metric.update_precomputed(float(loss.detach().item()), int(mask_bool.sum().item()))
            return loss
        else:
            # Fallback for non-torch or other custom logic
            ...
            if isinstance(metric, KLDMetric):
                ...
                masked_kl = kl_per_sample * mask_float
                loss = tlx.reduce_sum(masked_kl) / n_valid
