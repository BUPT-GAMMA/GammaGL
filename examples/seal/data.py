...
class SEALDataset(InMemoryDataset):
    def __init__(self, dataset, num_hops, neg_sampling_ratio=1.0, split='train'):
        self.data = dataset[0]
    ...
    def processed_file_names(self):
        return ['SEAL_train_data.npy', 'SEAL_val_data.npy', 'SEAL_test_data.npy']
        ...
        for data in chain(train_pos_data_list, train_neg_data_list,
            ...
            data.x = tlx.reshape(
                tlx.scatter_update(
                    ...
                    data.z + tlx.arange(0, data.z.shape[0]) * (self._max_z + 1),
                    tlx.ones((data.z.shape[0],))
                ),
    ...
    def extract_enclosing_subgraphs(self, edge_index, edge_label_index, y):
        ...
        for src, dst in edge_label_index.t().tolist():
            ...
            data = Graph(x=self.data.x[sub_nodes], z=z,
    ...
    def drnl_node_labeling(self, edge_index, src, dst, num_nodes=None):
        ...
        dist2src = tlx.convert_to_tensor(dist2src)

        dist2dst = shortest_path(adj_wo_src, directed=False, unweighted=True,
                                 indices=dst - 1)
        dist2dst = np.insert(dist2dst, src, 0, axis=0)
        dist2dst = tlx.convert_to_tensor(dist2dst)

        dist = dist2src + dist2dst
        dist_over_2, dist_mod_2 = dist // 2, dist % 2

        z = 1 + tlx.where(dist2src < dist2dst, dist2src, dist2dst)
        z += dist_over_2 * (dist_over_2 + dist_mod_2 - 1)
        z[src] = 1.
        z[dst] = 1.
        perm = tlx.mask_select(tlx.arange(0, z.shape[0]), tlx.is_nan(z))
        z = tlx.scatter_update(z, perm, tlx.zeros((perm.shape[0],), dtype=z.dtype))

        self._max_z = max(int(tlx.reduce_max(z)), self._max_z)
