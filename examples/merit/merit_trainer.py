...
def main(args):
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for _ in range(args.batch_size):
            ...
            data = {"feat1":graph1.x,"edge1":graph1.edge_index, "weight1":graph1.edge_weight, "num_node1":graph1.num_nodes,\
                "feat2":graph2.x,"edge2":graph2.edge_index, "weight2":graph2.edge_weight, "num_node2":graph2.num_nodes,
                }
        ...
        if epoch % args.eval_every_epoch == 0:
            net.set_eval()
            acc = evaluation(origin_graph.edge_index, origin_graph.edge_weight, diff_edge_index, diff_weight,\
                 features,net.gnn, train_mask, test_mask,labels,tlx.nn.PRelu(args.gnn_dim))
            if acc > best:
                best = acc
                patience_count = 0
            else:
