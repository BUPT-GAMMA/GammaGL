...
class SVDEncodingsDatasetBase:
    ...
    def cache_load_and_save(self, base_path, op, verbose):
        ...
        # 将 .pt 扩展名替换为 .npz
        svd_encodings_path = base_path/'svd_encodings.npy'
        
        if op == 'load':
            self._svd_encodings = tlx.files.load_npy_to_any(name=str(svd_encodings_path))
        elif op == 'save':
            ...
            tlx.files.save_any_to_npy(save_dict=self._svd_encodings, name=str(svd_encodings_path))
