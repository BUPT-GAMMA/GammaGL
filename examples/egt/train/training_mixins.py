...
class SaveModel(TrainingBase):
    ...
    def save_model(self, name):
        ...
        save_file = os.path.join(
            self.config.saved_model_path, name+'.npz')  # 替换 .pt 为 .npz
        ...
        if self.config.save_last_only and (self.state.last_saved_model_file is not None)\
            ...
            print(
                f'SAVE: removed old model file {self.state.last_saved_model_file}', flush=True)

        self.state.last_saved_model_file = save_file

    def on_batch_end(self, i, logs, training):
        super().on_batch_end(i, logs, training)
        if self.config.save_model_when != 'batch' or not training or not self.is_main_rank:
            return
        config = self.config
        scope = dict(batch=i)
    ...
    def on_epoch_end(self, logs, training):
        ...
        if config.save_model_when != 'epoch' or not self.is_main_rank:
            return
        scope = {}
        scope.update(self.state)
        scope.update(logs)
    ...
    def on_batch_begin(self, i, logs, training):
        ...
        if training and self.state.global_step <= self.config.lr_warmup_steps:
            new_lr = self.config.max_lr * \
                (self.state.global_step / self.config.lr_warmup_steps)
            self.optimizer.learning_rate = new_lr
            logs['lr'] = new_lr
    ...
    def on_batch_begin(self, i, logs, training):
        ...
        if training:
            ...
            if global_step <= lr_warmup_steps:
                new_lr = min_lr + (max_lr - min_lr) * \
                    (global_step / lr_warmup_steps)
            else:
                if self.config.cosine_halfwave:
                    new_lr = min_lr + (max_lr - min_lr) * np.cos(0.5 * np.pi * (
                        global_step - lr_warmup_steps) / (lr_total_steps - lr_warmup_steps))
                else:
                    new_lr = min_lr + (max_lr - min_lr) * (1 + np.cos(np.pi * (
                        global_step - lr_warmup_steps) / (lr_total_steps - lr_warmup_steps))) * 0.5
                new_lr = float(new_lr)
